...
import { useRoqClient, useRechargeFindFirst } from 'lib/roq';
...
function RechargeEditPage() {
  ...
  const { data, error, isLoading, mutate } = useRechargeFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: RechargeInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.recharge.update({
        data: values as RoqTypes.recharge,
        where: {
          id,
        },
      });
